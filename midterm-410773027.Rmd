...
```{r ans133}
sum(classSurvey$性別=="女")
str_split(classSurvey$課外活動,",")->classSurvey$activity
sum(classSurvey$性別=="女"&classSurvey$課外活動=="團隊活動")->a
sum(classSurvey$性別=="女"&classSurvey$課外活動=="團隊活動, 打工")->aa
sum(classSurvey$性別=="女"&classSurvey$課外活動=="團隊活動, 學會")->aaa
sum(classSurvey$性別=="女"&classSurvey$課外活動=="打工, 團隊活動")->aaaa
sum(a,aa,aaa,aaaa)->有參加團隊活動
女同學參加團隊活動的比例<-c(29/43,14/43)
```
...
rowSums(hwGrades2)->hwGrades$交作業次數
```
...
```{r ans15}
rowSums(hwGrades[,c(2:4)],na.rm = T)/3->
  hwGrades$平均成績
```
...
```{r ans16}
(hwGrades$學號 %in% classSurvey$學號) -> bbb
cbind(hwGrades$學號,bbb)

hwGrades[bbb,]->hwGradesSubsample
```
...
```{r ans17}
#hwGradesSubsample$學號%in% classSurvey$女性學號
#classSurvey%>%
#{cbind(.$學號,(.$性別=="女"))}
...
```{r ans18}
mean(hwGradesSubsample[(hwGradesSubsample$女性==F),]$平均成績)

#hwGradesSubsample[(hwGradesSubsample$女性==F),]$平均成績
